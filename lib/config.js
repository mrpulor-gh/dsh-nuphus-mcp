...
// Normalize user overrides: coerce numeric fields, validate arrays, and fall
// back to defaults on garbage so a typo can't silently break the MCP client.
export function resolveConfig(config = {}) {
  const merged = { ...DEFAULT_CLIENT_CONFIG, ...config };

  if (!Array.isArray(merged.args)) {
    merged.args = [...DEFAULT_CLIENT_CONFIG.args];
  }

  const timeout = Number(merged.toolCallTimeoutMs);
  merged.toolCallTimeoutMs =
    Number.isFinite(timeout) && timeout > 0
      ? Math.round(timeout)
      : DEFAULT_CLIENT_CONFIG.toolCallTimeoutMs;
